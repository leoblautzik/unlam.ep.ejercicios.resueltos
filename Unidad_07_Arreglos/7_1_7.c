...
int cargaEmpleados(int DNIs[], int sectores[], float sueldos[], int ceMax)
{
    int contador = 0;
    int dni;
...
void cuentaPorSector(int empleadosPorSector[], int sectores[], int ce)
{
    for(int i=0; i<ce; i++)
    {
...
void ordenaPorSector(int empleadosPorSector[])
{
    int nroSector[5];
    for(int i=0; i<5; i++)
        nroSector[i] = i+1;
...
float totalApagar(float sueldos[], int ce)
{
    float total=0;
...
int ingresarSector(int li, int ls){
